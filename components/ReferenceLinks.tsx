...
const ReferenceLinks: React.FC = () => {
  ...
  return (
    <div id="resources" className="bg-white py-24 relative overflow-hidden">
      {/* Decorative blobs */}
      <div className="absolute top-0 left-1/2 -ml-96 w-96 h-96 bg-blue-50 rounded-full blur-3xl opacity-50 pointer-events-none animate-pulse"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {resources.map((resource, index) => {
             // We can use a separate hook for each, or just use the parent's visibility + delay
             return (
              <a 
                key={resource.id} 
                href={resource.url} 
                ...
                className={`group relative bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-2 transition-all duration-500 ease-out transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}
              ...
              >
                ...
                <div className="flex items-center justify-between mb-6">
                  <span className="flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 shadow-lg shadow-blue-500/30 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
